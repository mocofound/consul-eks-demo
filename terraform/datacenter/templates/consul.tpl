...
User=consul
Group=consul
ExecStart=/usr/local/bin/consul agent -bind $LAN_IP -advertise-wan $WAN_IP -config-dir=/etc/consul.d/
...
services:
  ...
  currency_proxy:
    ...
    volumes:
      - "./currency.hcl:/config/currency.hcl"
    command: ["consul", "connect", "envoy", "-admin-bind", "$LAN_IP:19094", "-sidecar-for", "currency-${cluster-name}"]
